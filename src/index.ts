...
function buildHandlerPrompt(job: ReturnOnJob, reason: string, run: ReturnOnHandlerRun, eventJson: string): string {
	...
	return [
		...
		"- Do not run long foreground validations, CI watchers, servers, or sleeps from this handler. Start long commands in the background with logs/pid files and register/finalize an inherited-parent return_on watcher when more waiting is needed.",
