...
- High-confidence fragile background completion wrappers: when `set -e`/`errexit` is enabled, a command such as `( work; printf '%s\\n' "$?" > "$DONE" ) &` never writes the sentinel if `work` returns nonzero. Detection preserves multiline statement boundaries, recognizes combined option forms such as `set -o pipefail -o errexit`, accepts variable-backed or literal `.done`/`.status`/`.exit`/`.rc` targets, and considers safety control flow only inside the relevant background wrapper. Use `if work; then rc=0; else rc=$?; fi`, or an `EXIT` trap, before writing terminal status. Capture `$!` and watch process exit as a fallback so unexpected wrapper failure cannot leave a file-only watcher waiting until timeout.
